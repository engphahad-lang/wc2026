...
type Draft = { s1: string; s2: string; scorer: string; qualifier: string; assist: string; card: string }
...
export default function ParticipantPage() {
 ...
 async function savePrediction(match: Match) {
   ...
   const payload: any = {
     ...
     pred_assist: isLateKnockout ? (draft.assist || null) : null,
     pred_card: isLateKnockout ? (draft.card || null) : null,
   }
 ...
 function updateDraft(matchId: number, field: 'score1' | 'score2' | 'scorer' | 'qualifier' | 'assist' | 'card', val: string) {
   setDrafts(prev => ({
     ...prev,
     [matchId]: {
       ...prev[matchId],
       ...(field === 'score1' ? { s1: val }
         : field === 'score2' ? { s2: val }
         : field === 'scorer' ? { scorer: val }
         : field === 'qualifier' ? { qualifier: val }
         : field === 'assist' ? { assist: val }
         : { card: val }),
     }
   }))
 }
 ...
 function getDraft(match: Match): Draft {
   ...
   return {
     ...
     assist: drafts[match.id] !== undefined ? (d?.assist ?? '') : ((p as any)?.pred_assist || ''),
     card: drafts[match.id] !== undefined ? (d?.card ?? '') : ((p as any)?.pred_card || ''),
   }
 }
         ...
         return (
           <div className="space-y-3">
             {toShow.map(match => {
               ...
               const isKnockout = match.stage !== 'group'
               const isEarlyKnockout = match.stage === 'r16' || match.stage === 'qf'
               const isLateKnockout = match.stage === 'sf' || match.stage === 'final'
               const showScorerField = !isKnockout || isEarlyKnockout || isLateKnockout
               ...
               return (
                 ...
                 >
                   ...
                   {played && isLateKnockout && (match as any).assist && (
                     <div className="text-xs text-white/40 text-center">🎯 صانع الهدف: {(match as any).assist}</div>
                   )}
                   {played && isLateKnockout && (match as any).card && (
                     <div className="text-xs text-white/40 text-center">🟨 لاعب البطاقة: {(match as any).card}</div>
                   )}
                   ...
                   {played ? (
                     pred ? (
                       ...
                       >
                         ...
                         {(pred as any).pred_assist && <span className="text-white/40 mr-2">· 🎯 {(pred as any).pred_assist}</span>}
                         {(pred as any).pred_card && <span className="text-white/40 mr-2">· 🟨 {(pred as any).pred_card}</span>}
                     ...
                     pred ? (
                       <div className="bg-white/5 rounded-xl p-2.5 text-sm text-center text-white/50">
                         ...
                         {(pred as any).pred_assist && <span className="text-white/40 mr-1">· 🎯 {(pred as any).pred_assist}</span>}
                         {(pred as any).pred_card && <span className="text-white/40 mr-1">· 🟨 {(pred as any).pred_card}</span>}
                       </div>
                     ) : (
                     ...
                     <div className="space-y-2">
                       ...
                       {showScorerField && (
                         <input
                           ...
                           placeholder={isKnockout ? "مسجل الهدف (3 نقاط)" : "مسجل الهدف (اختياري)"}
                           className="w-full bg-navy border-2 border-white/20 focus:border-gold
                       ...
                       {isLateKnockout && (
                         <>
                           <input
                             type="text"
                             value={draft.assist}
                             onChange={e => updateDraft(match.id, 'assist', e.target.value)}
                             placeholder="صانع الهدف (3 نقاط)"
                             className="w-full bg-navy border-2 border-white/20 focus:border-gold
                                        rounded-xl py-2 px-3 text-sm outline-none transition-colors"
                           />
                           <input
                             type="text"
                             value={draft.card}
                             onChange={e => updateDraft(match.id, 'card', e.target.value)}
                             placeholder="لاعب البطاقة (3 نقاط)"
                             className="w-full bg-navy border-2 border-white/20 focus:border-gold
                                        rounded-xl py-2 px-3 text-sm outline-none transition-colors"
                           />
                         </>
                       ...
                       {isKnockout && draft.s1 !== '' && draft.s2 !== '' && draft.s1 === draft.s2 && (
