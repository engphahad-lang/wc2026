...
export default function HomePage() {
  ...
  async function handleLogin() {
    if (!selectedId || !pin) { setError('اختر اسمك وأدخل الرمز'); return }
    setChecking(true)
    setError('')
    ...
    if (data && data.pin === pin) {
      router.push(`/participant/${selectedId}`)
    } else {
      ...
      <header className="relative overflow-hidden bg-navy border-b border-gold/30">
        <div className="relative max-w-2xl mx-auto px-4 py-8 text-center">
          ...
          <div className="flex justify-center gap-4 mt-4 text-sm text-white/50">
            <span>🇺🇸 أمريكا</span><span>🇲🇽 المكسيك</span><span>🇨🇦 كندا</span>
          </div>
        </div>
      </header>
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        <div className="card space-y-4">
          <h2 className="text-lg font-bold text-gold">⚽ سجّل دخولك لإدخال توقعاتك</h2>
          <select value={selectedId} onChange={e => { setSelectedId(e.target.value); setError('') }}
            className="w-full bg-navy border-2 border-white/20 focus:border-gold rounded-xl py-3 px-4 text-white text-base outline-none">
          ...
          <input type="password" value={pin}
            ...
            placeholder="رمزك السري"
            className="w-full bg-navy border-2 border-white/20 focus:border-gold rounded-xl py-3 px-4 text-white text-base outline-none text-center tracking-widest"
          />
          {error && <p className="text-red-400 text-sm text-center">{error}</p>}
          <button onClick={handleLogin} disabled={checking || !selectedId || !pin}
        ...
        <div className="card space-y-3">
          ...
          {loading ? <div className="text-center py-8 text-white/40">جاري التحميل...</div> : (
            <div className="space-y-2">
              {leaderboard.map(row => (
                <div key={row.id} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${row.rank <= 3 ? 'bg-gold/10 border border-gold/30' : 'bg-white/5'}`}>
                  ...
                  <div className="text-left text-xs text-white/40 leading-tight">
                    <div>سابق: {row.prev_pts}</div><div>موقع: {row.site_pts}</div>
                  </div>
        ...
        <div className="text-center">
          <a href="/admin" className="text-xs text-white/20 hover:text-white/40">لوحة الإدارة</a>
        </div>
      </div>
    </div>
  )
